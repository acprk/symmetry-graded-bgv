...
which is what makes the ratios in Tables 4, 5, 7 and 8 head-to-head: only the environment
...
particular row of our Table 5 or the Discussion's artifact-reproduction paragraph, check out
vanilla HElib at `3e337a6`, apply the matching `GN*.patch`, and build with the artifact's own
`CMakeLists.txt` — this is exactly what `results/raw_logs/build_and_run.log` and
`zhao_thin24.log` record. We did **not** modify Zhao et al.'s code to run the aux-radix path
...
## Table 10 (`tab:rscan`): the r=1..32 exhaustive scan

This table does not compare against a baseline; it is a self-check that our own selector's
claim ("only r in {2,3,4,6} ever closes") is not an artifact of only having looked at small
r. Reproduce it with:
