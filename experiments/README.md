...
| `11_table_po2_composed_backtoback.sh` | Tables 12 and 13 (`tab:po2`, `tab:composed`): scalar axis alone against composed, back to back in one context per row | ~10–40 min per row |
...
being where `../baselines/setup.sh` clones Zhao et al.'s artifact.
...
two. They are kept because Tables 12 and 13, which are stage-level rather than end-to-end, have
